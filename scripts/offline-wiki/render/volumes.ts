...
function story(logicalId: string, summaries: Map<string, ReviewedSummary>): string {
  const summary = summaries.get(logicalId);
  return summary
    ? `<section><h3>故事摘要</h3><p>${textToHtml(summary.summary)}</p><p class="meta">原创摘要 · ${escapeHtml(summary.reviewer.name)} 审核</p></section>`
    : `<section><h3>故事摘要</h3><p class="missing">尚无经审核的原创故事摘要。</p></section>`;
}
...
function renderCharacters(
  ...
  summaryMap: Map<string, ReviewedSummary>,
...
function renderDivergentUniverse(
  input: VolumeRenderInput,
  summaryMap: Map<string, ReviewedSummary>,
  assetMap: Map<string, RenderAsset>,
): string {
  const items = input.catalog.divergentUniverse;
  const body = `${toc(items)}${items.map((item) => `
    <article class="entry" id="${anchorFor(item.logicalId)}">
      <h2>${escapeHtml(item.name)}</h2>
      <p class="meta">${escapeHtml(item.kind)} · ${escapeHtml(item.releaseId)}</p>
      ${image(item.logicalId, "差分宇宙图片", assetMap)}
      <p>${textToHtml(item.description)}</p>
      ${story(item.logicalId, summaryMap)}
      ${sources(item.provenance)}
      <p class="return"><a href="#top">返回目录</a></p>
    </article>`).join("")}${items.length === 0 ? "<p class=\"missing\">当前版本尚无经审核的差分宇宙资料。</p>" : ""}`;
  return htmlDocument("差分宇宙图鉴", `${input.catalog.release.gameVersion} · ${input.catalog.release.id}`, body);
}
...
function renderIndex(input: VolumeRenderInput): string {
  ...
  const rows = [
    ...
    ["差分宇宙", catalog.divergentUniverse.length, "04-差分宇宙图鉴.html"],
  ] as const;
  ...
  const entityIndex = [
    ...
    ["差分宇宙", "04-差分宇宙图鉴.pdf", catalog.divergentUniverse],
  ...
  )).join("")}</tbody></table><p>经审核故事摘要：${catalog.summaryCoverage.reviewed}；缺少：${catalog.summaryCoverage.missing}。</p></section>${changeSection}${entityIndex}`;
...
export function renderVolumes(input: VolumeRenderInput): RenderedVolume[] {
  ...
  return [
    ...
    { filename: "04-差分宇宙图鉴.html", title: "差分宇宙图鉴", html: renderDivergentUniverse(input, summaryMap, assetMap) },
  ];
}
