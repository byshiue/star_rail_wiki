...
function renderIndex(input: VolumeRenderInput): string {
  ...
  )).join("")}</tbody></table><p>故事摘要：${catalog.summaryCoverage.available}（人工复核 ${catalog.summaryCoverage.reviewed}；自动生成 ${catalog.summaryCoverage.autoGenerated}）；缺少：${catalog.summaryCoverage.missing}。</p></section>${changeSection}${entityIndex}`;
