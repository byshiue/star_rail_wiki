...
test("evaluates a real four-person 4.4 team with ally and enemy targets and evidence", async ({ page }) => {
  ...
  await page.getByRole("combobox", { name: "布洛妮娅作战再部署技能等级" }).selectOption("15");
  await page.getByRole("combobox", { name: "佩拉领域压制技能等级" }).selectOption("15");
  ...
  await expect(page.getByText("单体队友增伤 +82.5%", { exact: true })).toBeVisible();
  await expect(page.getByText("全体敌人减防 +45%", { exact: true })).toBeVisible();
  ...
  const evidenceCases = [
    ...
    ["conditional · 条件生效", "查看单体队友增伤来源", "解除指定我方单体的1个负面效果，并使该目标立即行动，造成的伤害提高33%→82.5%，持续1回合", "ability:110102@4.4-cn-2026-08-21", true],
    ["conditional · 条件生效", "查看全体敌人减防来源", "【通解】状态下，敌方目标防御力降低30%→45%，持续2回合", "ability:110603@4.4-cn-2026-08-21", true],
  ] as const;
  for (const [category, buttonName, originalText, revision, hasSkillLevel = false] of evidenceCases) {
    ...
    if (hasSkillLevel) {
      await expect(dialog).toContainText("等级 15（明确选择；最高 15）");
    }
  ...
  await page.reload();
  await expect(page.getByRole("combobox", { name: "布洛妮娅作战再部署技能等级" })).toHaveValue("15");
  await expect(page.getByRole("combobox", { name: "佩拉领域压制技能等级" })).toHaveValue("15");
  ...
  await expect(page.getByText("单体队友增伤 +82.5%", { exact: true })).toBeVisible();
  await expect(page.getByText("全体敌人减防 +45%", { exact: true })).toBeVisible();
  for (const buttonName of ["查看单体队友增伤来源", "查看全体敌人减防来源"]) {
    await page.getByRole("region", { name: /conditional · 条件生效/ })
      .getByRole("button", { name: buttonName }).click();
    const dialog = page.getByRole("dialog", { name: "效果证据" });
    await expect(dialog).toContainText("等级 15（明确选择；最高 15）");
    await dialog.getByRole("button", { name: "关闭" }).click();
  }
