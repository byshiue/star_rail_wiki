...
import type { EffectEvidence, TeamEvaluation } from "../effects/evaluateTeam";
    ...
    <section className="results-panel" aria-labelledby="evaluation-title">
      ...
      {evaluation?.warnings.length ? (
        <section className="evaluation-warnings" aria-labelledby="warnings-title">
          ...
          <ul>{evaluation.warnings.map((warning, index) => <li key={`${warning.code}:${index}`}>{warning.message}</li>)}</ul>
        </section>
      ) : null}
    </section>
  );
